...
# TODO: things to be modified in CodeGen
# 1. remove the case having three AODs stacked on the same site and simplify the logic
# 2. input code contains the exact qubit location Don't need to do conversion 
# 3. (optional) don't drop qubits to SLM after each movement
# 4. output code contains start time and end time for each instruction
# 5. support multiple AODs: decide in codeGen or ZAC?
#     1) prevent multiple AODs from operating on the same row or column: Pipeline manner: a. We only have one AOD picking up atoms from the storage zone, b. by row
#     2) the AOD that picks up qubits first should drop first
# 6. support compute-store arch. Input code contains a list of Rydberg regions. Each region is defined by the upper-left point and lower-right point
# 7. how to park AOD qubits in the storage zone? move the to the middle of two rows in the storage zone
...
class Inst(ABC):
    """abstract class of Enola instructions.
    ...
    def write_code(
        ...
        if global_dict['full_code']:
            ...
            # get the current state of Enola
            curr = {}
            curr['qubits'] = [
                {
                    'id': q.id,
...
class ComboInst():
    ...
    instructions or Enola instructions.
    """
...
class CodeGen():
    ...
    corresponding to a Enola instruction defined above. 
    """
...
class Animator():
    ...
    def __init__(self,
        ...
        Args:
            ...
            show_graph (bool, optional): whether show the graph on the right
                side of the Enola in the animation. Defaults to False.
    ...
    def setup_canvas(self, scaling_factor: int):
        ...
        if self.show_graph:
            # 1 row 2 col, Enola on the left, the graph on the right
...
if __name__ == "__main__":
    ...
    parser.add_argument('--arch_param', help='hardware parameters for Enola', type=str, default = "hardware_spec/default.json")
