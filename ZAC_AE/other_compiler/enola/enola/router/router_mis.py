...
def route_qubit_mis(chip_dim: tuple, n_q: int, index_list_gate:int, list_gates: list, qubit_mapping: list, \
                    routing_strategy: str, reverse_to_initial: bool, move_back_to_initial_layout_after_circuit_execution: bool,\
    ...
    while remain_graph:
        ...
        for i in execute_gates:
            ...
            # ! do some extra stuff for collecting qubit moveing distance
        #     dis = math.dist((vectors[i][0], vectors[i][1]), (vectors[i][2], vectors[i][3]))
        #     tmp_moving_distance.append(dis)
        # list_qubit_move_distance.append(tmp_moving_distance)
    ...
    # print("data")
    # print(data)
    if index_list_gate + 1 < len(list_gates) or move_back_to_initial_layout_after_circuit_execution:
        if reverse_to_initial:
            ...
            # todo: find final mapping here
            # final_mapping = [qubit_mapping[i] for i in range(n_q)]
            # set_aod_qubit = set()
            print("[INFO] Enola: Find a new mapping for the next Rydberg stage")
            list_aod_qubit = list(set_aod_qubit)
            # list_gate_next_layers = []
            # for i in range(index_list_gate + 1, len(list_gates)):
            #     list_gate_next_layers += list_gates[i]
            # list_gate_next_layers = list_gates[index_list_gate+1:]
            ...
            # list_qate_distance = [[ math.dist(initial_mapping[gate[0]], initial_mapping[gate[1]]) for gate in tmp_list_gate] for tmp_list_gate in list_gate_next_layers]
            ...
            while len(list_aod_qubit) > 0:
                vectors = []
                # for q in list_aod_qubit:
                #     vectors.append((qubit_mapping[q][0], initial_mapping[q][0], qubit_mapping[q][1], initial_mapping[q][1]))
    ...
    # print(len(layers))
    # print(layer_time)
    # print("layer_gate_number")
    # print(layer_gate_number)
    return data, initial_mapping, time_placement, list_qubit_move_distance, list_qate_distance