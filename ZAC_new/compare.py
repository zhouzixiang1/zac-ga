"""ZAC_new 五方对比表：ZAC 真值 / GA v1a / FABLE / ZAC_new-B(penalty) / ZAC_new-A(ga)。

全部只读——旧实验的数据一个字节不动。指标：
    duration ratio（对 ZAC 归一，<1 = 更快）、重排批数、平均原子/批、
    fidelity、保真度 geomean；ZAC_new 两行额外汇总"χ 预演 vs 实际批数"
...
SYSTEMS = [  # (标签, 结果目录, 是否本实验)
    ...
    ("ZAC_new-B", ZNEW_ROOT / "results/repro_penalty", True),
    ("ZAC_new-A", ZNEW_ROOT / "results/repro_ga", True),
]
...
def ledger_stats(d: Path) -> str:
    """ZAC_new 批次账本：χ 预演 vs 实际执行的一致率（一行摘要）。"""
    rows = []
...
def main() -> None:
    if len(sys.argv) > 1:  # 允许换本实验目录（消融用）
        SYSTEMS[3] = ("ZAC_new-B", ZNEW_ROOT / sys.argv[1], True)
        SYSTEMS[4] = ("ZAC_new-A", ZNEW_ROOT / sys.argv[2], True)
    ...
    md = ("# ZAC_new 五方对比（18 电路，全部同底盘只换放置/路由）\n\n"
          "## 时长 ratio（对 ZAC 归一，<1 更快）与批次\n\n"
          + "\n".join(lines) + "\n\n## 保真度\n\n" + "\n".join(flines) + "\n\n"
          "## χ 预演对账（ZAC_new 专属）\n\n"
          + f"- ZAC_new-B: {ledger_stats(SYSTEMS[3][1])}\n"
          + f"- ZAC_new-A: {ledger_stats(SYSTEMS[4][1])}\n")
