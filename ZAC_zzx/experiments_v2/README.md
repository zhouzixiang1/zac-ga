...
`legacy/pilot`，只用于回归和故障定位，不能进入新主表。清理当前工作树后，
清单使用两种明确的保留状态：

- `working_tree`：文件仍保留在当前 tip，测试直接校验其 SHA-256。现保留的
  `experiments/results/qmap_iccad_v32.json` 属于此类。
- `git_history_only`：旧产物已从当前 tip 有意删除，清单记录最后包含该内容的
  40 位 `git_commit`；测试通过 `git_commit:path` 读取历史 blob 并校验 SHA-256。

两种状态都只表示历史证据的可核验保留方式，不改变
`accepted_by_schema2_aggregator=false` 的排除契约。
...
ROOT=../fidelity-lookahead-v2/artifacts/native-ga-v1/tuning-quality-v1
