...
class RuntimeProtocolTests(unittest.TestCase):
    ...
    def test_schedule_is_deterministic_and_balanced(self):
        left = build_balanced_schedule(["a", "b"], repetitions=5)
        right = build_balanced_schedule(["a", "b"], repetitions=5)
        self.assertEqual(left, right)
        jobs = left["jobs"]
        self.assertEqual(len(jobs), 2 * 4 * 5)
    ...
    def test_schedule_tampering_fails_even_when_resealed(self):
        schedule = build_balanced_schedule(["a", "b"], repetitions=5)
    ...
    def test_summary_only_computes_strict_ratio_for_equal_ledgers(self):
        ...
        self.assertAlmostEqual(row["M3"]["speedup_vs_m2"], 102 / 52)
        self.assertAlmostEqual(row["M4"]["speedup_vs_m2"], 102 / 42)
        summary = result["paired_speedup_vs_m2"]["M4"]
        self.assertEqual(summary["n"], 1)
        self.assertAlmostEqual(
            summary["geometric_mean_speedup_vs_m2"], 102 / 42)
        self.assertAlmostEqual(summary["bootstrap95_low"], 102 / 42)
