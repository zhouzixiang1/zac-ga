...
REAL_ARTIFACT_ROOT = ROOT.parent / "fidelity-lookahead-v2" / "artifacts"
...
class BaselineRawReplayTests(unittest.TestCase):
    ...
    def test_registered_archive_inventory_is_lineage_explicit(self):
        ...
        unfrozen = audit_baseline_raw_replay(
            plan, [REAL_ARTIFACT_ROOT], methods=["M1"])
        frozen_ready = {
            (entry["dataset"], entry["circuit"])
            for entry in frozen["entries"] if entry["state"] == "ready"
        }
        unfrozen_ambiguous = {
            (entry["dataset"], entry["circuit"])
            for entry in unfrozen["entries"] if entry["state"] == "ambiguous"
        }
        self.assertTrue(frozen_ready <= unfrozen_ambiguous)
