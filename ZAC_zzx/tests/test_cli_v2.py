...
class TestCliPlan(unittest.TestCase):
    ...
    def test_legacy_pilot_inventory_is_hash_locked_and_excluded(self):
        ...
        self.assertEqual(
            set(inventory["retention_contract"]),
            {"working_tree", "git_history_only"},
        )
        repo = ROOT.parent
        for artifact in inventory["artifacts"]:
            path = repo / artifact["path"]
            retention_state = artifact["retention_state"]
            self.assertIn(retention_state, inventory["retention_contract"])
            if retention_state == "working_tree":
                self.assertTrue(path.is_file())
                self.assertNotIn("git_commit", artifact)
                self.assertEqual(sha256_file(path), artifact["sha256"])
                continue

            self.assertEqual(retention_state, "git_history_only")
            self.assertFalse(path.exists())
            git_commit = artifact["git_commit"]
            self.assertRegex(git_commit, r"^[0-9a-f]{40}$")
            historical_blob = subprocess.run(
                ["git", "show", f"{git_commit}:{artifact['path']}"],
                cwd=repo,
                check=True,
            ...
            self.assertEqual(
                hashlib.sha256(historical_blob).hexdigest(),
                artifact["sha256"],
            )
