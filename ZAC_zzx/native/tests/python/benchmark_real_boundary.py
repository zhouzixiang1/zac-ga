"""Real-circuit Python-reference versus ABI3 exact-boundary benchmark.
...
def assert_parity(reference: dict[str, Any], native: dict[str, Any]
    ...
    return {
        **checks,
        ...
        # unique fitness work; ABI3 returns the incumbent directly.
...
def benchmark(*, qasm_path: Path = DEFAULT_QASM,
    ...
    if not native_available():
        raise RuntimeError("ABI3 zac_native_core is not installed")
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join((
        "# ABI3 exact real-boundary benchmark",
        "",
        ...
        f"{reference[primary]['median'] / 1e6:.3f} ms, ABI3 C++ "
