...
前 3 行来自 qmap examples 套件（ZAC 为同会话现场跑），后 3 行来自 hpca
套件（ZAC 为冻结真值）：
...
| qft_n29 | 110 | 401 → 217 | 736 → 667 | **0.777** |
| knn_n31 | 77 | 161 → 88 | 238 → 123 | **0.631** |

（①轮数的证据形态：轮数 = rydberg 指令数 = ASAP 层数，两法一致；
1q 门全程零额外搬运批次。②人次的下降几乎全部来自回程相消失。③qft_n29
的批数腰斩但人次只降 9%——批数收益来自着色放置，人次收益受限于链式
电路"新原子必须入场"的硬需求，两个证据列恰好分工。）
