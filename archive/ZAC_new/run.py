"""ZAC_new 入口（前台）：与 GA/FABLE 的 run.py 流程相同，仅创建 ZAC_new。

用法：
    ZAC/.venv/bin/python ZAC_new/run.py ZAC_new/exp_setting/zac_new_toy.json   # 冒烟
    ZAC/.venv/bin/python ZAC_new/run.py ZAC_new/exp_setting/zac_new_repro.json # 18 电路
...
# ZAC_new 文件夹自身：本地 zac/（ZAC 源码副本）和 znew/（实验包）都从这里找
...
from znew.zac_new import ZAC_new  # noqa: E402     换过发动机的 ZAC


def resolve(p: str) -> str:
    """把任务单里的相对路径锚到 ZAC_new 文件夹——换机器、换目录都不会迷路。"""
    q = Path(p)
...
if __name__ == "__main__":
    ...
    for benchmark in benchmark_set:
        ...
        for zac_setting in exp_spec["zac_setting"]:
            ...
            # 创建编译器并跑完整流水线（解析→调度→点名→布局(ZAC_new在这里)→路由→校验）
            compiler = ZAC_new()
            ...
            # 批次账本落盘：放置 χ 预演 vs 路由实际批数（一致性对账的核心数据）
            ledger = {
                "placer_preview": [list(map(float, row)) for row in compiler.znew_placer_preview],
                "route_log": compiler.znew_route_log,
            }
