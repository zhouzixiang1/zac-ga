"""FABLE 入口（前台）：与 GA/run.py 流程相同，仅创建 ZAC_FABLE。

用法：
    ZAC/.venv/bin/python FABLE/run.py FABLE/exp_setting/fable_toy.json    # 玩具冒烟
...
# FABLE 文件夹自身：本地 zac/（ZAC 源码副本）和 fable/（实验包）都从这里找
...
from fable.zac_fable import ZAC_FABLE  # noqa: E402   换过发动机的 ZAC


def resolve(p: str) -> str:
    """把任务单里的相对路径锚到 FABLE 文件夹——换机器、换目录都不会迷路。"""
    q = Path(p)
...
if __name__ == "__main__":
    ...
    for benchmark in benchmark_set:
        ...
        for zac_setting in exp_spec["zac_setting"]:
            ...
            # 创建编译器并跑完整流水线（解析→调度→点名→布局(Fable在这里)→路由→校验）
            compiler = ZAC_FABLE()
            compiler.parse_setting(s)                    # 读开关：placer=fable/zac + 搜索旋钮
